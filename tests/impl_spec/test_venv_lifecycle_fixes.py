...
def test_migrate_gitignore_normalizes_entries(tmp_path, monkeypatch, test_settings):
    """migrate() normalizes .gitignore: recognizes .venv/ as equivalent to .venv.

    The .gitignore has '.venv/' — the fix should normalize to '.venv' and not add
    a duplicate. We verify by checking the file content actually changed from original.
    """
    monkeypatch.chdir(tmp_path)
    base = tmp_path

    # Pre-create .gitignore with trailing-slash variant of .venv
    original = ".venv/\n"
    (base / ".gitignore").write_text(original)
    (base / "requirements.txt").write_text("requests\n")
    ...
    content = (base / ".gitignore").read_text()
    # The fix's ensure_gitignore should have read the file, found .venv/ which
    # normalizes to .venv, and therefore NOT added a bare .venv entry.
    # Crucially: the file content should differ from original (migrate touched it).
    # If migrate never touched the file, content == original (the XPASS case).
    assert content != original, (
        "migrate() should modify .gitignore. File unchanged means not processed."
    )
    # And no duplicate .venv variants
    lines = [line.strip() for line in content.strip().splitlines()]
    venv_lines = [entry for entry in lines if entry.strip("/") == ".venv"]
    assert len(venv_lines) == 1, (
        f"Should have exactly one .venv variant, got: {venv_lines}. "
        f"Full content:\\n{content}"
    )


def test_migrate_gitignore_normalizes_leading_slash(
    tmp_path, monkeypatch, test_settings
):
    """migrate() recognizes /.venv as equivalent to .venv.

    The .gitignore has '/.venv' — the fix should normalize and not duplicate.
    We verify by checking the file content actually changed from original.
    """
    monkeypatch.chdir(tmp_path)
    base = tmp_path

    # Pre-create .gitignore with leading-slash variant
    original = "/.venv\n"
    (base / ".gitignore").write_text(original)
    (base / "requirements.txt").write_text("requests\n")
    ...
    content = (base / ".gitignore").read_text()
    # The fix's ensure_gitignore should have processed this file.
    # If content == original, migrate never touched it.
    assert content != original, (
        "migrate() should modify .gitignore. "
        "File unchanged means gitignore was not processed."
    )
    # And no duplicate .venv variants
    lines = [line.strip() for line in content.strip().splitlines()]
    venv_lines = [entry for entry in lines if entry.strip("/") == ".venv"]
