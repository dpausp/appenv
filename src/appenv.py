...
class AppEnv:
    ...
    def run(self, command, argv):
        ...
        if self.settings.profile:
            ...
            print(f"Command profiling enabled, output at {profile_output}")
    ...
    def init(self, args=None, remaining=None):
        ...
        log.debug("Init, chdir to %s", target)
        os.chdir(target)
    ...
    def reset(self, args=None, remaining=None):
        ...
        if self.venv_link.exists() and not self.venv_link.is_symlink():
            print(f"Removing old {self.venv_link} ...")
            shutil.rmtree(self.venv_link)
    ...
    def _find_profile(self, args_file, message_prefix="Showing"):
